...
class Token(object):
    def __init__(self, *parents):
        """
        :param parents: tokens which lead to this token
        """
        self.parents = set([id(parent) for parent in parents])
...
#     SEPARATOR = "_"
#
#     def __init__(self, name='no name', show=True, fire=True, logger=logging):
#         self.name = name
#         """ If several tokens are firing by the same transition,
#             the result is then a new token whose name is an union
#             of the previous token's names, separate by a SEPARATOR
#
#             .. Warning:: A token's name must not have an SEPARATOR in its name
#         """
#         self.show = show
#         self.fire = fire
...
#             of the strings in the associated list, i.e one of these strings is in ``token.name.split(cls.SEPARATOR)``.
...
#
#     def __str__(self):
#         return ', '.join(self.name.split(self.SEPARATOR))
#
#     def __repr__(self):
...
#         """
#         return set(self.name.split(self.SEPARATOR)).issubset(names)
#
